...
const reportCollab = async (req, res) => {
  ...
  try {
    ...
    const check_connected_collabs = removeOneMatch(
      findUser.dataValues.connected_collabs,
      collab_user_id,
      collab_id_to_block
    );

    const check_rejected_collabs = removeOneMatch(
      findUser.dataValues.rejected_collabs,
      collab_user_id,
      collab_id_to_block
    );

    const check_outbox_collabs = removeOneMatch(
      findUser.dataValues.outbox,
      collab_user_id,
      collab_id_to_block
    );
    ...
    return res.status(200).json({ message: "Collab reported successfully", status: 200 });
